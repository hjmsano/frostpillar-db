...
フィルタもコピーされ、すべての入力は**ちょうど 1 回だけ**読み取られます。したがって getter や `Proxy` がバリデータとクエリエンジンに異なる値を見せることはできません:
...
スキップモードでも、軽量なセキュリティバリデータはすべての書き込みで実行されます。予約キー、循環参照、`maxDepth` を超える深いネスト、プレーンでないオブジェクト値（クラスインスタンス、`Date`、`Map`、`Set`、`Object.create(proto)`）、および保存不可能なリーフ型（`bigint`、`function`、`symbol`、`undefined`）は `ValidationError` で拒否されます。関数と Symbol を拒否する理由は、これらが参照型だからです。保存されたドキュメントは呼び出し側のオブジェクトを共有し続けるため、書き込み後の変更が以降の読み取り結果に反映されてしまいます。スキップモードが無効化するのはサイズ計算（バイト数、キー数、文字列・キー長の上限）です。
...
可変幅量指定子数の上限は、エンジンが消費幅を選択できるアトムの連鎖(例: `^.?.?….?aaa…a$`)への対策です。この形ではどの量指定子もアトムを繰り返さないため、繰り返し・ネスト・選択のいずれのチェックにも掛かりません。しかし各アトムは独立した分岐点となり、マッチ失敗時には最大 2^k 通りの経路が探索されます。上限を 8 とすることで、この形の探索経路を約 256 通りに抑えます。**最小繰り返し回数がゼロであることは本質ではありません** — `{1,2}` のアトムを連鎖させた場合、各アトムは必ずマッチするものの、1 文字か 2 文字かを選べるため、同じ 2^k の爆発を許します(`+` や `{n,}` はさらに悪化します)。したがって最小値と最大値が異なる量指定子はすべて計上し、固定幅の `{n}` のみを対象外とします。この上限は隣接する連続数ではなくパターン全体の総数で数えます — 可変幅アトムの間に挟まれた固定幅アトム(`.?\w.?\w…`)は常に同じ幅を消費するため、分岐を刈り取らないからです。
