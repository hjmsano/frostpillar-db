...
**Limit:** The same `MAX_DISTINCT_COUNT` (100,000) cap as `.distinct()` applies, since the implementation must remember every distinct value seen (even though it discards the values themselves) to detect duplicates. Exceeding the cap throws `ValidationError` at the identical boundary as `.distinct()`.
...
- Throws `ValidationError` if a `$countDistinct` accumulator's unique-value count within any single group would exceed `MAX_DISTINCT_COUNT` (100,000).
